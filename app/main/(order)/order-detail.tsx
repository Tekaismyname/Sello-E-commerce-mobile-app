...
export default function OrderDetailScreen() {
  ...
  const handleBuyAgain = async (orderToBuy: Order) => {
    ...
    Alert.alert(
      "Mua lại đơn hàng",
      "Bạn có muốn thêm tất cả sản phẩm của đơn hàng này vào giỏ hàng?",
      [
        { text: "Không", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              for (const item of orderToBuy.items) {
              ...
              Alert.alert(
                "Thành công",
                "Đã thêm tất cả sản phẩm vào giỏ hàng. Đi đến giỏ hàng?",
                [
                  { text: "Để sau" },
                  { text: "Giỏ hàng", onPress: () => router.push("/main/cart" as Href) }
                ]
              );
            } catch (err: any) {
              Alert.alert("Lỗi", err.message ?? "Có lỗi xảy ra khi thêm sản phẩm vào giỏ hàng.");
            }
          }
        }
      ]
        ...
        <>
          ...
          <View className="border-t border-[#E1E7EF] bg-white px-4 py-3 gap-2">
            ...
            {(order.status === "delivered" || order.status === "cancelled" || order.status === "returned") && (
              ...
              >
                <Text className="text-[16px] font-extrabold text-white">Mua lại đơn hàng</Text>
              </Pressable>
            )}
            <Pressable
